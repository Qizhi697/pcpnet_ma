...
def parse_arguments():
    ...
    parser.add_argument('--refine', type=str, default='./models/my_single_scale_normal_stn2_model.pth',
...
def train_pcpnet(opt):
    ...
    params_filename = os.path.join(opt.outdir, '%s_stn2_params.pth' % (opt.name))
    model_filename = os.path.join(opt.outdir, '%s_stn2_model.pth' % (opt.name))
    desc_filename = os.path.join(opt.outdir, '%s_stn2_description.txt' % (opt.name))
...
def reverse_mapping_cell_determined(pred_map, norm, targetc, index_del):
    B = pred_map.shape[0]
    M = pred_map.shape[1]
    targetc_map = norm_to_grid(targetc, B, M).cuda()
    targetc_map = targetc_map * index_del[:B, :, :]
    pre = torch.zeros(B, 3)
    for i in torch.arange(0, B):
        ind = torch.argmax(pred_map[i, :, :])
        indc = torch.argmax(targetc_map[i, :, :])
        if ind == indc:
            pre[i, :] = targetc[i, :]
        else:
            pre[i, :] = norm[ind, :]
...
def reverse_mapping_cell_determined8k(pred_map, norm, targetc, index_del):
    B = pred_map.shape[0]
    M = pred_map.shape[1]
    targetc_map = norm_to_grid(targetc, B, M).cuda()
    targetc_map = targetc_map * index_del[:B, :, :]
    ...
    for i in torch.arange(0, B):
        ind = torch.argmax(pred_map[i, :, :])
        indc = torch.argmax(targetc_map[i, :, :])
        if int(ind == indc) | int(torch.abs(ind - indc) == 1) | int(torch.abs(ind - indc) == 33) | int(
                torch.abs(ind - indc) == 31) | int(torch.abs(ind - indc) == 34):
            pre[i, :] = targetc[i, :]
        else:
            pre[i, :] = norm[ind, :]
    return pre.cuda()
