...
def load_shape(point_filename, normals_filename, curv_filename, pidx_filename):
    ...
    sys.setrecursionlimit(
        int(max(1000, round(pts.shape[0] / 10))))  # otherwise KDTree construction may run out of recursions
...
class SequentialShapeRandomPointcloudPatchSampler(data.sampler.Sampler):
    ...
    def __iter__(self):
        ...
        for shape_ind in shape_inds:
            ...
            global_patch_inds = self.rng.choice(range(start, end), size=min(self.patches_per_shape, end - start),
                                                replace=False)
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shape_list_filename, patch_radius, points_per_patch, patch_features,
                 seed=None, identical_epochs=False, use_pca=True, center='point', point_tuple=1, cache_capacity=1,
    ...
    def __getitem__(self, index):
        ...
        if self.point_tuple > 1:
            patch_tuples = torch.zeros(self.points_per_patch * len(self.patch_radius_absolute[shape_ind]),
                                       3 * self.point_tuple, dtype=torch.float)
            for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        normals_filename = os.path.join(self.root,
                                        self.shape_names[shape_ind] + '.normals') if self.include_normals else None
        curv_filename = os.path.join(self.root,
                                     self.shape_names[shape_ind] + '.curv') if self.include_curvatures else None
