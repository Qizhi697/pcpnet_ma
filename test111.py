...
from pcpnet0 import PCPNet, MSPCPNet
import numpy as np
import cv2
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            while test_fraction_done <= train_fraction_done and test_batchind + 1 < test_num_batch:
                ...
                # np.savetxt('extract.txt',data[0][uu,:,:].numpy())
...
def compute_loss(pred, target, outputs, output_pred_ind, output_target_ind, output_loss_weight, patch_rot, normal_loss):
    ...
    for oi, o in enumerate(outputs):
        ...
        # if loss.item() > 0.2:
        #     print(1)
        #     tt = torch.min((o_pred - o_target).pow(2).sum(1), (o_pred + o_target).pow(2).sum(1))
        #     uu = torch.argmax(tt).cpu().numpy()
        #     np.savetxt('pred.txt', o_pred[uu, :].cpu().numpy())
        #     np.savetxt('target.txt', o_target[uu, :].cpu().numpy())
