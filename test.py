...
def parse_arguments():
    ...
    parser.add_argument('--refine', type=str, default='./models/my_single_scale_normal_no1k0.01_2model.pth',
    ...
    parser.add_argument('--patch_radius', type=float, default=[0.1], nargs='+',
    ...
    parser.add_argument('--points_per_patch', type=int, default=1000, help='max. number of points per patch')
...
def train_pcpnet(opt):
    ...
    params_filename = os.path.join(opt.outdir, '%s_27_params.pth' % (opt.name))
    model_filename = os.path.join(opt.outdir, '%s_27_model.pth' % (opt.name))
    desc_filename = os.path.join(opt.outdir, '%s_27_description.txt' % (opt.name))
    ...
    pcpnet = pcpnet.cuda()
    # pcpnet = torch.nn.DataParallel(pcpnet)
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            dist = torch.abs(torch.bmm(grid_norm, points))
            # tt = (dist < 0.1).type(torch.uint8)
            # num = tt.sum(axis=-1)
            # num[:, index_null] = 0
            # point_num = num.reshape(opt.batchSize, M, M)
            # point_num = point_num.cpu().numpy()
            # mean = torch.mean(dist, dim=-1).unsqueeze(-1)
            mean = (torch.ones(opt.batchSize, M ** 2, 1) * 0.01).cuda()
            ...
            print('[%s %d: %d/%d] %s loss: %f' % (
                opt.name, epoch, train_batchind, train_num_batch - 1, green('train'), loss1.item()))
            ...
            while test_fraction_done <= train_fraction_done and test_batchind + 1 < test_num_batch:
                ...
                # print('[%s %d: %d/%d] %s loss: %f,loss_normal: %f' % (
                #     opt.name, epoch, train_batchind, train_num_batch - 1, blue('test'), loss.item(), loss_norm.item()))
                test_writer.add_scalar('loss', loss.item(),
                                       (epoch + test_fraction_done) * train_num_batch * opt.batchSize)
                ll.append(loss_norm.item())
        # scheduler.step()
        result = str(np.array(ll).mean())
