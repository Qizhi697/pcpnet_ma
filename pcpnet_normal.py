...
class PointNetfeat(nn.Module):
    def __init__(self, num_scales=1, num_points=500, use_point_stn=True, use_feat_stn=True, sym_op='max',
        ...
        self.conv0a = torch.nn.Conv1d(6 * self.point_tuple, 64, 1)
...
class PCPNet(nn.Module):
    ...
    def forward(self, grid_norm, weight_null, x1, tc, tp, M):
        feat, points, target_c, trans, trans2, pointfvals = self.feat(x1, tc, tp)
        # if target_c is not None:
        #     adjust_ind = torch.nonzero(target_c[:, 2] < 0)
        ...
        x = x.reshape(points.size(0), 1024, M, M)
