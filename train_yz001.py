...
def parse_arguments():
    ...
    parser.add_argument('--refine', type=str, default='./models/my_single_scale_normal_stnmlp_model.pth',
...
def train_pcpnet(opt):
    ...
    params_filename = os.path.join(opt.outdir, '%s_stnmlp_params.pth' % (opt.name))
    model_filename = os.path.join(opt.outdir, '%s_stnmlp_model.pth' % (opt.name))
    desc_filename = os.path.join(opt.outdir, '%s_stnmlp_description.txt' % (opt.name))
