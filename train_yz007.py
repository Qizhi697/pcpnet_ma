...
from pcp_yz007 import PCPNet, MSPCPNet
import numpy as np
import cv2
...
def parse_arguments():
    ...
    parser.add_argument('--refine', type=str, default='./models/my_single_scale_normal_stnmlp007_model.pth',
...
def train_pcpnet(opt):
    ...
    params_filename = os.path.join(opt.outdir, '%s_stnmlp007_params.pth' % (opt.name))
    model_filename = os.path.join(opt.outdir, '%s_stnmlp007_model.pth' % (opt.name))
    desc_filename = os.path.join(opt.outdir, '%s_stnmlp007_description.txt' % (opt.name))
